...
function typeHandler(type, symbol) {
    let screen = document.getElementById('screenSpan');
    switch (type) {
        case 'digit':
        ...
        case 'clear':
            ...
            break;
        case 'result':
            screen.innerText = evaluate(compile(screen.textContent));
            break;
    }
}
...
function clickHandler(event) {
    let screen = document.getElementById('screenSpan');
    //get span element in screen
    let target = event.target;
    if (target.classList.contains('digit')) {
        typeHandler('digit', target.textContent);
    } else if (target.classList.contains('operation')) {
        typeHandler('operation', target.textContent);
    } else if (target.classList.contains('bracket')) {
        typeHandler('bracket', target.textContent);
...
window.onload = function () {
    ...
    document.getElementById('clearButton').addEventListener('click', clickHandler);
    buttons.addEventListener('click', clickHandler);

    document.onkeydown = function (event) {
        let key = event.key;
        if (key >= 0 && key <= 9) {
            typeHandler('digit', key);
        } else if (['+', '-', '*', '/'].includes(key)) {
            typeHandler('operation', key);
        } else if (['(', ')'].includes(key)) {
            typeHandler('bracket', key);
        }
        switch (key) {
            case 'Enter':
                typeHandler('result');
